...
void Player_NOKDPS::getMoves(GameState &state, const MoveArray &moves, std::vector<Action> &moveVec)
{
	...
	for (IDType u(0); u < moves.numUnits(); ++u)
	{
		...
		const Unit &closestUnit(ourUnit.canHeal() ? state.getClosestOurUnit(_playerID, u) : ourUnit.type().isDetector() ? state.getClosestEnemyUnit(_playerID, u, false)
																														: state.getClosestEnemyUnit(_playerID, u, true));
